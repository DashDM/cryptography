...
std::string HexToBin(const std::string& hexNumber) {
    ...
    conv['e'] = "1110";
    conv['f'] = "1111";
    for (auto& i : hexNumber) {
        binNumber += conv[i];
    }

    /*std::cout << "|Hash_To_Binary:\n";
...
std::vector<long long> HashToVector(const std::string& hexHash) {
    ...
    int j = 0, l = binHash.size();
    for (size_t i = 0; i <l;) {
        res.push_back((std::bitset<16>(binHash.substr(i, 16)).to_ulong()));
        i = i + 16;
        j++;
    }
    return res;
...
long long* HashToArray(const std::string& hexHash) {
    ...
    int j = 0, l = binHash.size();
    for (size_t i = 0; i < l;) {
        res[j] = (std::bitset<16>(binHash.substr(i, 16)).to_ulong());
        i = i + 16;
        j++;
    }
    return res;
...
std::string DecToBin(unsigned long long dec) {
    std::string binary = std::bitset<16>(dec).to_string();
    return binary;
}


std::vector<long long> DecToVector(long long dec ) {
    std::vector<long long> res;
    std::string binHash = DecToBin(dec);
    int j = 0, l = binHash.size();
    for (size_t i = 0; i < l;) {
        res.push_back((std::bitset<16>(binHash.substr(i, 16)).to_ulong()));
        i = i + 16;
